...
// .skill/scripts/src/rebuild-index.ts
...
// .skill/scripts/src/lib/frontmatter.ts
...
function getStringField(fm, key) {
  const v = fm[key];
...
function walkAllDirectories(root) {
  const all = [root];
  const stack = [root];
...
var TEMPLATES_DIR = (0, import_node_path3.resolve)(__dirname, "../assets/templates");
function today() {
...
function readTemplate(name) {
  const p = (0, import_node_path3.join)(TEMPLATES_DIR, name);
  if (!(0, import_node_fs3.existsSync)(p)) {
    process.stderr.write(`error: template not found: ${p}
`);
    process.exit(2);
  }
  return (0, import_node_fs3.readFileSync)(p, "utf8");
}
function folderPages(folder) {
  return listMarkdownFiles(folder).filter((p) => !EXCLUDED_STEMS.has(stemOf(p))).sort();
}
function folderSubfolders(folder) {
...
function renderFolderIndex(vault, folder, dateStr) {
  ...
  const pageList = pages.length > 0 ? pages.map((p) => `- [[${stemOf(p)}]]`).join("\n") : "- \uFF08\u306A\u3057\uFF09";
  const subList = subs.length > 0 ? subs.map((s) => `- [${(0, import_node_path3.basename)(s)}/](${(0, import_node_path3.basename)(s)}/index.md)`).join("\n") : "- \uFF08\u306A\u3057\uFF09";
  const out = tpl.replaceAll("{{date}}", dateStr).replaceAll("{{folder_title}}", title).replaceAll("{{page_list}}", pageList).replaceAll("{{subfolder_list}}", subList);
  const dest = (0, import_node_path3.join)(folder, "index.md");
  (0, import_node_fs3.writeFileSync)(dest, out, "utf8");
  process.stdout.write(`rebuilt: ${relPath(vault, dest)}
`);
}
function renderRootIndex(vault, dateStr) {
  ...
  for (const p of allMd) {
    ...
    const parent = p.slice(0, p.length - (0, import_node_path3.basename)(p).length - 1);
  ...
  const out = tpl.replaceAll("{{date}}", dateStr).replaceAll("{{sources_conversations_list}}", fmt(buckets["source-conversation"])).replaceAll("{{sources_urls_list}}", fmt(buckets["source-url"])).replaceAll("{{entities_list}}", fmt(buckets.entity)).replaceAll("{{concepts_list}}", fmt(buckets.concept));
  (0, import_node_fs3.writeFileSync)((0, import_node_path3.join)(vault, "index.md"), out, "utf8");
...
function main() {
  ...
  const dateStr = today();
  if (target) {
    const tPath = (0, import_node_path3.resolve)(vault, target);
    if (!(0, import_node_fs3.existsSync)(tPath) || !(0, import_node_fs3.statSync)(tPath).isDirectory()) {
