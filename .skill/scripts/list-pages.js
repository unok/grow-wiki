...
// .skill/scripts/src/list-pages.ts
...
function main() {
  ...
  const root = subpath ? (0, import_node_path3.resolve)(vault, subpath) : vault;
  const allMd = walkAllMarkdownFiles(root).sort();
  const pages = [];
  for (const md of allMd) {
    let text;
    try {
      text = (0, import_node_fs3.readFileSync)(md, "utf8");
    } catch {
      continue;
    }
    const { frontmatter } = parseFrontmatter(text);
    const st = (0, import_node_fs3.statSync)(md);
    const dirRel = relPath(vault, (0, import_node_path3.dirname)(md));
    pages.push({
      path: relPath(vault, md),
      basename: (0, import_node_path3.basename)(md).replace(/\.md$/, ""),
      dir: dirRel === "." ? "" : dirRel,
      frontmatter,
