...
function getStringListField(fm, key) {
  const v = fm[key];
  if (Array.isArray(v)) {
...
var L1_WARN = 20;
var L1_ERROR = 40;
var L2_WARN_LINES = 300;
var L2_WARN_CHARS = 8e3;
var L2_ERROR_LINES = 500;
var L2_ERROR_CHARS = 15e3;
var EXCLUDED_STEMS = /* @__PURE__ */ new Set(["index", "overview", "log", "README"]);
var CITATION_HEADINGS = /* @__PURE__ */ new Set(["\u51FA\u5178", "Sources", "References", "\u53C2\u8003", "Citations"]);
var MISC_DIR_NAMES = /* @__PURE__ */ new Set(["misc", "etc", "others"]);
...
function relPath(vault, p) {
  if (p === vault) return ".";
  return (0, import_node_path3.relative)(vault, p).split(import_node_path3.sep).join("/");
}
...
function main() {
  ...
  const warnings = [];
  const errors = [];
  const addWarn = (rule, message) => {
    warnings.push({ rule, message });
  };
  const addError = (rule, message) => {
  ...
  for (const md of allMd) {
    const text = (0, import_node_fs3.readFileSync)(md, "utf8");
  ...
  for (const meta of pageMeta.values()) {
    if (meta.lines > L2_ERROR_LINES || meta.chars > L2_ERROR_CHARS) {
      addError(
        "L2",
        `${meta.rel}: ${meta.lines} lines / ${meta.chars} chars (error, threshold ${L2_ERROR_LINES} lines / ${L2_ERROR_CHARS} chars)`
      );
    } else if (meta.lines > L2_WARN_LINES || meta.chars > L2_WARN_CHARS) {
      addWarn(
        "L2",
        `${meta.rel}: ${meta.lines} lines / ${meta.chars} chars (warn, threshold ${L2_WARN_LINES} lines / ${L2_WARN_CHARS} chars)`
      );
    }
  }
  ...
  for (const d of allDirs) {
    const idxPath = (0, import_node_path3.join)(d, "index.md");
    ...
    const extra = [...linked].filter((s) => !actual.has(s) && !EXCLUDED_STEMS.has(s) && !allStems.has(s)).sort();
    if (missing.length > 0) {
      addError(
        "L3",
  ...
  for (const d of allDirs) {
    if (d === vault) continue;
    if (!MISC_DIR_NAMES.has((0, import_node_path3.basename)(d).toLowerCase())) continue;
    const subdirs = listSubdirectories(d);
    if (subdirs.length === 0) continue;
    const rel = relPath(vault, d);
    const names = subdirs.slice(0, 5).map((s) => (0, import_node_path3.basename)(s)).join(", ");
    const more = subdirs.length > 5 ? "..." : "";
    addError(
      "L7",
      `${rel}/: ${(0, import_node_path3.basename)(d)}/etc/others \u4E0B\u306B\u30B5\u30D6\u30D5\u30A9\u30EB\u30C0\u3092\u4F5C\u3089\u306A\u3044 (${subdirs.length} subfolders: ${names}${more}) \u2192 \u89AA\u30D5\u30A9\u30EB\u30C0\u306B\u65B0\u3057\u3044\u30D5\u30A9\u30EB\u30C0\u3092\u4F5C\u3063\u3066\u5206\u985E`
    );
  }
  for (const d of allDirs) {
    const subdirs = listSubdirectories(d);
    ...
    addError(
      "L5",
      `${rel}/: \u30B5\u30D6\u30D5\u30A9\u30EB\u30C0\u304C\u5B58\u5728\u3059\u308B\u306E\u306B\u76F4\u4E0B\u306B ${directPages.length} \u30DA\u30FC\u30B8\u3042\u308B (${stems.join(", ")}${more}) \u2192 \u30B5\u30D6\u30D5\u30A9\u30EB\u30C0\u306B\u79FB\u52D5\u3059\u308B\u304B misc/ \u3078`
    );
  }
  ...
  for (const d of allDirs) {
    ...
    if (idxDate) {
      ...
      if (newer.length > 0) {
        ...
        addWarn(
          "L4",
          `${idxMeta.rel}: last_updated=${idxLast}, ${newer.length} newer pages (${newer.slice(0, 5).join(", ")}${more})`
        );
      }
    }
    for (const link of extractWikilinks(stripCode(idxMeta.text))) {
